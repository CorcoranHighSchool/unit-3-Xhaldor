...
func calculateArea (x : Double, y : Double) ->Double?{
    ...
    guard x > 0 && y > 0 else {
        print("One or more values given are less than 0")
        return nil
    }
    return x * y;
}
...
func add (alpha : Int?, beta : Int?) ->Int? {
    //validate that all values are not nil
    guard let a : Int = alpha, let b : Int = beta else {
        print("One of more of the values given are nil")
        return nil
    }
    return a + b
}
...
func createdUser() -> User? {
    ...
    let user = User(firstName: first, lastName: last, age: age)
    return user
}
var dude = createdUser()
if let user = dude{
    print("The user's name is \(user.firstName) \(user.lastName), and they are \(user.age) years old")
}
/*:
